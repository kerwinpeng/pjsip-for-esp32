...
static void on_syntax_error(pj_scanner *scanner)
{
    ...
    PJ_LOG(2,("sample", "syntax error %d", PJ_EINVAL));
}
            ...
            PJ_TRY {
                while (!pj_scan_is_eof(&scanner)) {
                    ...
                    if (cmd->sc_cnt >= PJ_CLI_MAX_SHORTCUTS) {
                        PJ_LOG(2,("sample", "syntax error %d", PJ_CLI_ETOOMANYARGS));
                    }
                    /* Check whether the shortcuts are already used */
                    if (get_cmd_name(cli, &cli->root, &str)) {
                        PJ_LOG(2,("sample", "syntax error %d", PJ_CLI_EBADNAME));
                    }
