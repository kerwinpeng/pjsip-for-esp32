						   ...
						   int *p_tp_type)
{
    ...
    if (tp_flag & PJSIP_TRANSPORT_IPV6 && parent) {
        transport_names[i].type = (pjsip_transport_type_e)(parent | PJSIP_TRANSPORT_IPV6);
    } else {
					      ...
					      pjsip_transport *tp )
{
    ...
    TRACE_((THIS_FILE, "Transport %s registered: type=%s, remote=%s:%d",
	    ...
	    pj_sockaddr_has_addr(&tp->key.rem_addr)?
				pj_addr_string(&tp->key.rem_addr):"",
	    pj_sockaddr_has_addr(&tp->key.rem_addr)?
...
PJ_DEF(pj_status_t) pjsip_tpmgr_destroy( pjsip_tpmgr *mgr )
{
    ...
    {
	transport *tp_ref;
	tp_ref = (transport *)pj_hash_this(mgr->table, itr);
