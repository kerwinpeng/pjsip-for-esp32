...
#include <pjlib-util/util_errno.h>
#include <pj/hash.h>
#include <pj/pool.h>
#include <pj/pj_os.h>
#include <pj/rand.h>
...
{
	pj_time_val timeout_timer_val = { (64*PJSIP_T1_TIMEOUT)/1000,
					 (64*PJSIP_T1_TIMEOUT)%1000 };
	pj_time_val td_timer_val = { PJSIP_TD_TIMEOUT/1000, PJSIP_TD_TIMEOUT%1000 };
	pj_time_val t2_timer_val = { PJSIP_T2_TIMEOUT/1000, PJSIP_T2_TIMEOUT%1000 };
	/* Timer timeout value constants */
	pj_time_val t1_timer_val = { PJSIP_T1_TIMEOUT/1000, PJSIP_T1_TIMEOUT%1000 };
	pj_time_val t4_timer_val = { PJSIP_T4_TIMEOUT/1000, PJSIP_T4_TIMEOUT%1000 };
    pj_pool_t *pool;
    pj_status_t status;
				 ...
				 pjsip_rx_data *rdata)
{
    ...
    PJ_LOG(5,(tsx->obj_name, "Incoming %s in state %d", 
	      pjsip_rx_data_get_info(rdata), tsx->state));
    pj_log_push_indent();
...
static pj_status_t tsx_on_state_calling( pjsip_transaction *tsx, 
				         pjsip_event *event )
{
	pj_time_val t2_timer_val = { PJSIP_T2_TIMEOUT/1000, 
...
static pj_status_t tsx_on_state_completed_uas( pjsip_transaction *tsx, 
                                               pjsip_event *event)
{
	pj_time_val t4_timer_val = { PJSIP_T4_TIMEOUT/1000, 
