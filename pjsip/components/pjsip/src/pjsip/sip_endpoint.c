...
static void pool_callback( pj_pool_t *pool, pj_size_t size )
{
    ...
    PJ_LOG(2,("sample", "no memory error"));
}
...
PJ_DEF(pj_status_t) pjsip_endpt_create(pj_pool_factory *pf,
				       const char *name, pjsip_endpoint **p_endpt)
{
    pj_status_t status;
    pj_pool_t *pool;
    ...
    *p_endpt = NULL;
    ...
    if (name != NULL) {
        ...
        pj_strset2(&local_host, "localhost");
	    pj_strdup_with_null(endpt->pool, &endpt->name, &local_host);
    }
