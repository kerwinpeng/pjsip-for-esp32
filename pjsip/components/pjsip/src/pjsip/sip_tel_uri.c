...
PJ_DEF(pjsip_tel_uri*) pjsip_tel_uri_create(pj_pool_t *pool)
{
    ...
    pjsip_uri_vptr *tel_uri_vptr = PJ_POOL_ZALLOC_T(pool, pjsip_uri_vptr);
    pjsip_tel_uri *uri = PJ_POOL_ZALLOC_T(pool, pjsip_tel_uri);
    pj_memcpy(tel_uri_vptr, &tel_uri_vptr1, sizeof(tel_uri_vptr1));
...
{
    pj_cis_buf_t *cis_buf = (pj_cis_buf_t *)pj_calloc(1, sizeof(pj_cis_buf_t));
    pj_status_t status;
			    ...
			    pj_bool_t parse_params)
{
    ...
    const pjsip_parser_const_t *pc = pjsip_parser_const();
    pj_str_t pjsip_PH_CTX_STR = { "phone-context", 13 };
    pj_str_t pjsip_EXT_STR = { "ext", 3 };
    pj_str_t pjsip_ISUB_STR = { "isub", 4 };
