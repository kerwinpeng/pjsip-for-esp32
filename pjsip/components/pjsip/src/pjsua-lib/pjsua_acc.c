						...
						pj_bool_t renew)
{
	...
	if (status == PJ_SUCCESS) {
			...
			pjsua_var->acc[acc_id].via_addr.host.slen > 0)
		{
			...
			/* Choose local interface to use in Via
			*/
				   ...
				   const void **p_tp)
{
    ...
    tfla2_prm.local_if = (flag & PJSIP_TRANSPORT_RELIABLE);
						  ...
						  const pj_str_t *suri)
{
    ...
    status = pjsua_acc_get_uac_addr(acc_id, pool, suri, &addr,
                                    &tp_type, &secure, NULL);
	printf("pjsua_acc_create_uac_contact status=%d 0\n", status);
    if (status != PJ_SUCCESS)
		return status;
	...
	printf("pjsua_acc_create_uac_contact tp_type=%d 1\n", tp_type);
	...
	printf("pjsua_acc_create_uac_contact transport_param=%s 2\n", transport_param);
