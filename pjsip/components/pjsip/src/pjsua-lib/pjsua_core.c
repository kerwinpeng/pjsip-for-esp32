...
pjsua_data *pjsua_var = NULL;

PJ_DEF(pjsua_data *) pjsua_get_var(void)
{
    return pjsua_var;
}
...
static int init_data()
{
	...
	pjsua_var = (pjsua_data *)pj_malloc(sizeof(pjsua_data));
	if (!pjsua_var)
	{
		printf("pjsua_core init data memory alloc failed.\n");
		return 1;
	}
...
PJ_DEF(pj_status_t) pjsua_create(void)
{
	...
	if (rc != PJ_SUCCESS)
	{
		printf("pjsua_create init data failed %p.\n", pjsua_var);
		return 1;
	}
     ...
     */
    status = pjsip_endpt_create(&pjsua_var->cp.factory, "localhost", 
				&pjsua_var->endpt);
    if (status != PJ_SUCCESS) {
		pj_log_pop_indent();
				...
				const pjsua_media_config *media_cfg)
{
	...
	for (ii=0; ii<pjsua_var->ua_cfg.thread_cnt; ++ii) {
		...
		status = pj_thread_create(pjsua_var->pool, tname,
					  &worker_thread_timer,
					  NULL, 0, 0, &pjsua_var->thread[ii], 18);
	    } else {
		status = pj_thread_create(pjsua_var->pool, tname,
	    ...
	    status = pj_thread_create(pjsua_var->pool, tname, &worker_thread,
				      NULL, 0, 0, &pjsua_var->thread[ii], 19);
#endif
	    if (status != PJ_SUCCESS)
		goto on_error;
	}
    ...
    PJ_LOG(3,(THIS_FILE, "pjsua version %s initialized", 
			 pj_get_version()));
				       ...
				       pj_sockaddr *p_pub_addr)
{
    ...
    *p_sock = sock;
    ...
    PJ_LOG(4,(THIS_FILE, "SIP UDP socket reachable at %s:%d",
	      pj_addr_string(p_pub_addr), (int)pj_sockaddr_get_port(p_pub_addr)));
					    ...
					    pjsua_transport_id *p_id)
{
    ...
    if (type==PJSIP_TRANSPORT_UDP || type==PJSIP_TRANSPORT_UDP6) {
		...
		*/
		...
		pj_ansi_strcpy(hostbuf, pj_addr_string(&pub_addr));
...
PJ_DEF(void) pjsua_dump(pj_bool_t detail)
{
	...
	for (j = 0; j < tp_cnt; ++j) {
	    ...
	    PJ_LOG(3,(THIS_FILE, " %s: %s",
		      "UDP", pj_sockaddr_print(&tpinfo.sock_info.rtp_addr_name,
					addr_buf,
					sizeof(addr_buf), 3)));
	}
    }
