...
char* get_libsrtp_errstr(int err)
{
    char *msg = (char *)pj_calloc(20, sizeof(char));
    pj_ansi_snprintf(msg, sizeof(char) * 20, "libsrtp error %d", err);
    return msg;
}
...
PJ_DEF(pj_status_t) pjmedia_srtp_init_lib(pjmedia_endpt *endpt)
{
	...
	if (err != srtp_err_status_ok) {
	    PJ_LOG(4, (THIS_FILE, "Failed to initialize libsrtp."));
...
# endif
    if (err != srtp_err_status_ok) {
	PJ_LOG(4, (THIS_FILE, "Failed to deinitialize libsrtp"));
    }
				       ...
				       pjmedia_transport **p_tp)
{
    ...
    pj_status_t status;
    unsigned i;
    pjmedia_transport_op transport_srtp_op1 =
    {
        &transport_get_info,
    ...
    pjmedia_transport_op *transport_srtp_op = PJ_POOL_ZALLOC_T(pool, pjmedia_transport_op);
	pj_memcpy(transport_srtp_op, &transport_srtp_op1, sizeof(pjmedia_transport_op));
...
PJ_DEF(pj_status_t) pjmedia_transport_srtp_stop(pjmedia_transport *srtp)
{
    ...
    if (err != srtp_err_status_ok) {
	PJ_LOG(4, (p_srtp->pool->obj_name, "Failed to dealloc RX SRTP context"));
    }
    err = srtp_dealloc(p_srtp->srtp_tx_ctx);
    if (err != srtp_err_status_ok) {
	PJ_LOG(4, (p_srtp->pool->obj_name, "Failed to dealloc TX SRTP context"));
    }
...
static void srtp_rtp_cb(pjmedia_tp_cb_param *param)
{
	...
	status = pjmedia_transport_srtp_start((pjmedia_transport*)srtp,
					      &tx, &rx);
	if (status != PJ_SUCCESS) {
	    PJ_LOG(5,(srtp->pool->obj_name, "Failed to restart SRTP."));
	...
	PJ_LOG(5,(srtp->pool->obj_name,
		  "Failed to unprotect SRTP, pkt size=%d", size));
    } else {
	cb = srtp->rtp_cb;
	cb2 = srtp->rtp_cb2;
...
static void srtp_rtcp_cb( void *user_data, void *pkt, pj_ssize_t size)
{
    ...
    if (err != srtp_err_status_ok) {
	PJ_LOG(5,(srtp->pool->obj_name,
		  "Failed to unprotect SRTCP, pkt size=%d", size));
    } else {
	cb = srtp->rtcp_cb;
	cb_data = srtp->user_data;
    }
						       ...
						       int *pkt_len)
{
    ...
    if (err != srtp_err_status_ok) {
	PJ_LOG(5,(srtp->pool->obj_name,
		  "Failed to unprotect SRTP, pkt size=%d", *pkt_len));
    }
