                                           ...
                                           pjmedia_event_publish_flag flag)
{
    ...
    if (flag & PJMEDIA_EVENT_PUBLISH_POST_EVENT) {
        ...
        if (mgr->pub_ev_queue) {
            ...
            mgr->pub_ev_queue = (event_queue *)pj_calloc(1, sizeof(event_queue));
            mgr->pub_ev_queue->is_full = PJ_FALSE;
            ...
            do {
                ...
                if (status != PJ_SUCCESS && err == PJ_SUCCESS)
	            err = status;
            } while(mgr->pub_ev_queue->head != mgr->pub_ev_queue->tail || mgr->pub_ev_queue->is_full);

            pj_free(mgr->pub_ev_queue);
