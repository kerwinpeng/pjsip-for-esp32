...
#define THIS_FILE   "transport_srtp_sdes.c"
			       ...
			       pjmedia_transport **p_keying)
{
	...
	pjmedia_transport_op *sdes_op = PJ_POOL_ZALLOC_T(srtp->pool, pjmedia_transport_op);
	pj_memcpy(sdes_op, &sdes_op1, sizeof(pjmedia_transport_op));
    sdes->op = sdes_op;
    sdes->user_data = srtp;
