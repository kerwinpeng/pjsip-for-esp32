 ...
 */
PJ_INLINE(void *) pj_malloc(pj_size_t size)
{
    return heap_caps_malloc(size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
}

/**
 * Alloc memory with initialized zero value.
 *
 * @param num   Number of continuing chunks of memory to allocate.
 * @param size	Size in bytes, of a chunk of memory to allocate.
 *
 * @return allocated memory pointer.
 */
PJ_INLINE(void *) pj_calloc(pj_size_t num, pj_size_t size)
{
    void *buf = NULL;
 ...
 *
 */
PJ_INLINE(void) pj_free(void *ptr)
{
    free(ptr);
}
