...
static void (*atexit_func[4])(void);
...
static pj_status_t init_mutex(pj_mutex_t *mutex, const char *name, int type)
{
	...
	rc = pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_NORMAL);
        ...
        (defined(ESP32_PLATFORM) && ESP32_PLATFORM!=0) || \
...
PJ_DEF(pj_status_t) pj_rwmutex_unlock_write(pj_rwmutex_t *mutex)
{
    ...
    xSemaphoreGive(*(mutex->write_lock->sem));
    return PJ_SUCCESS;
}
#else
struct pj_rwmutex_t
				   ...
				   pj_sem_t **ptr_sem)
{
    ...
    sem->sem = (sys_sem_t*)pj_pool_alloc(pool, sizeof(sys_sem_t*));
    if (!sem->sem){
        PJ_LOG(6, (THIS_FILE, "pj_sem_create: out of memory 1"));
        return PJ_RETURN_OS_ERROR(pj_get_native_os_error());
    }

    *sem->sem = xSemaphoreCreateMutex();
    if (!(*sem->sem)){
        PJ_LOG(6, (THIS_FILE, "pj_sem_create: out of memory 2"));
        return PJ_RETURN_OS_ERROR(pj_get_native_os_error());
    }
    ...
    if (strchr(name, '%')) {
	    pj_ansi_snprintf(sem->obj_name, PJ_MAX_OBJ_NAME, name, sem->sem);
    } else {
...
PJ_DEF(pj_status_t) pj_sem_wait(pj_sem_t *sem)
{
    ...
    PJ_LOG(6, (sem->obj_name, "Semaphore: thread %s is waiting",
			      pj_thread_this()->obj_name));
    result = xSemaphoreTake(*sem->sem, 100/portTICK_PERIOD_MS);
    if (result == pdTRUE) {
...
PJ_DEF(pj_status_t) pj_sem_trywait(pj_sem_t *sem)
{
    ...
    PJ_LOG(6, (sem->obj_name, "Semaphore: thread %s is waiting",
			      pj_thread_this()->obj_name));
    result = xSemaphoreTake(*sem->sem, 0);
    if (result == pdTRUE) {
        PJ_LOG(6, (sem->obj_name, "Semaphore acquired by thread %s",
                    pj_thread_this()->obj_name));
        return PJ_SUCCESS;
